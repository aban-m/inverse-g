...
and the resulting graph gives $g(K) + g(K; v_1) = 4 + 2 = 6$ groups by \hthref{eujoin}.
Call this new graph $K_{VI}$.
We cannot extend $K_{VI}$: $K_{VI} \to f$ as well as both $b \to K_{VI}$ give too many groups by \hthref{eustick}.
...
It is easy to see that the only possible extensions are $f \to L$ and $L \to f$. According to \hthref{eustic}, the former yields $2g(L) = 10$ groups, and the latter $5 + 2 = 7$ groups. So we have another admissible case.
...
By \hthref{euppq} we see that there is no $r \to p^2$ since $r \ge 5$.
...
\scase{C.1} \emph{$qrs$ has precisely one edge $q \to r$.} We claim that $n$ is inadmissible.\nopagebreak
...
Consider the homomorphism $\phi : H_0 \to \aut{\cyc{s}} \cong \cyc{s - 1}$ defining the semidirect product.
