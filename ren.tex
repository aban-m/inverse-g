...
\usepackage[backend=bibtex, style=numeric, url=false]{biblatex}
...
\NewDocumentCommand{\hold}{}{\text{Holder}}
...
% finishing \hold case work in lambda(n) = 4 [requires TikZ]
...
%The function $g(n)$, counting isomorphism types of groups of order $n$, has various interesting arithmetical properties. It is common knowledge that $g(p) = 1$ for all primes, for example - but a simple argument shows that $g(15) = 1$ as well, even though $15$ is not prime. This raises the question: For which $n$ do we have $g(n) = 1$? Equivalently, since there is a unique cyclic group for of a g3iven order: Which numbers $n$ have the property that every group of order $n$ is cyclic?
%
%This question n now be generalized in two different directions: First, given a group-theoretic property $\mathcal{P}$, we can aim to characterize aall $n$ such that all groups of order $n$ has property $P$. It is common to call these $\mathcal{P}$ numbers, so we have the question of characterizing cyclic numbers, abelian numbers, nilpotent numbers, and so on. The Feit-Thompson theorem, in these terms, is equivalent to stating that all odd \emph{numbers} are solvable. In {\cite{nilnumb}}, these questions are answered, in addition to the question of characterizing solvable numbers.
%
%Secondly, we can simply go from $g(n) = 1$ to $g(n) = k$, for fixed $k$, and find all $n$ that satisfy this equation. This is the direction we intend to pursue. In fact, Miller answers this problem for $k = 2, 3$ in {\cite{miller1}} and $k = 4, 5$ in {\cite{miller2}}. The same result appears has been re-derived multiple times, see for i is conjectured that this equation is solvable for every $k$, and a strengthened form of this conjecture is that it can in fact be solved in odd \emph{squarefree} integers. This has been verified for $n$ up to $10^x$.
%
%The behavior of $g(n)$ depends strongly on the factorization of $n$. It is known that [FORMULA1] and [FORMULA2], for example ($\mu$ denotes the largest exponent in the unique factorization of $n$ into prime powers). Moreover, $g$ is supermultiplicative: We have $g(ab) \ge g(a)g(b)$. By exploiting these two facts, along with enumerations for $g(n)$ when $n$ has a few number of prime factors, we restrict the class of potential solutions to a few cases, which we then analyze.  A first step, for example, is observing that since $g(p^4) \ge 14$ for all primes $p$, all solutions to $g(n) \in \{6, 7\}$ must be fourth-power-free.
%
%Explicit formulae for calculating $g(n)$ based on its factorization are rare. Perhaps the most remarkable is \hold's formula, when $n$ is squarefree. Multiple mathematicians worked on enumerating groups of order $n$ when $n$ has a small number of divisors, such as $p^k$ for $k \le 4$, $p^2 q^2$, and so on. A summary of these results, as well as a historical overview of the problem, can be found in {\cite{bettinafour1}}. We see that 


\section{\hold graphs}
\subsection{\hold's formula}
...
\begin{fact}[\textbf{\hold's formula}]\thlabel{euholder} In this notation, we have
	\begin{equation*}
...
It is therefore natural to define the \emph{\hold graph} of a squarefree integer $n$ to be unlabeled directed graph whose vertices are the prime factors, and where we draw an edge $p \rightarrow q$ precisely when $p$ is related to $q$. Observe also that when the out-degree of every vertex is at most 1, it follows that $g(n)$ depends solely on the \hold graph, and therefore we are justified in speaking of $g(\Gamma).$ We call both $n$ and $\Gamma$ \emph{regular} in this case. We will occasionally treat directed acyclic graphs $\Gamma$ with only the vertices whose out-degree exceeds 1 labeled. In this case, we can still speak of $g(\Gamma)$ without ambiguity, as it will be independent from the labels of the rest of the vertices. Observe also that a given (unlabeled) directed acyclic graph is always realizable as the \hold graph of some odd squarefree integer, via Dirichlet's theorem on arithmetic
%\begin{center}\begin{tikzpicture}
%%	\scalebox{0.7}{
	%	\node[point] (a) {};
	%	\node[point,right=of a, label=7] (hepta) {};
	%	\node[point, right=of hepta, yshift=10mm] (b) {};
	%	\node[point, right=of hepta, yshift=-10mm] (c) {};
	%	\path[->, thick] (a) edge (hepta)
	%		(hepta) edge (c)
	%		(hepta) edge (b);
	%%	}
%\end{tikzpicture}\linebreak A {sufficiently labeled} graph. \end{center}

We generalize this to arbitrary $n$ as follows. The vertices are now the prime powers, and an edge $p^\alpha \rightarrow q^\beta$ exists precisely when $q^j \equiv 1 \Mod{p^i}$ for some $1 \le i \le \alpha$ and $1 \le j \le \beta$. The edge will be labeled $(i, j)$ where $j$ is the smallest such integer, and $i$ the largest, and we drop $i$ if $\alpha = 1$ and similarly for $j$. We call this the \emph{generalized \hold graph} for $n.$ These are not necessarily acyclic, and the labels will not be of much interest to us. In analyzing the solutions to $g(n) = k$, we will make use of this concept. To simplify notation further, we write $p^2 \dashrightarrow q$ to mean $p \parallel q - 1$. On the other hand, we write $q \dashrightarrow p^2$ to mean $q \mid p + 1$ but $q \nmid p - 1$ (that is, $p$ has order $2$ modulo $q$). We call these ``weak arrows.``

%\emph{Remark.} From Dirichlet's theorem on arithmetic progression see that every directed acyclic graph is the \hold graph of infinitely many squarefree integers: Ultimately, choosing the next prime in the factorization amounts to finding a prime satisfying some linear congruences modulo primes, and this is always possible.
In the following, we let $S(\pi, \Gamma)$ denote the summand corresponding to the subset $\pi$ in a \hold graph $\Gamma,$ and each factor of it will be denoted $h_\Gamma(p, \pi).$ Moreover, we will call vertices $q$ with no edge $q \rightarrow v$ terminal, and vertices $p$ with no edge $v \rightarrow p$ as initial. A vertex with out-degree of at most 1 will be called regular.
...
\begin{thm}\thlabel{eufrobenius}
	...
	\begin{enumerate}\listspace
		...
		\item $a$ and $b$ are coprime and arithmetically independent - that is, the generalized \hold graph of $n$ decomposes into two disjoint subgraphs corresponding to $a$ and $b.$
...
\end{prop}
\begin{proof}
	This follows immediately from considering the generalized \hold graph of $n$ and letting the $n_i$ be the numbers corresponding to the connected component of at least two vertices, and $m$ the product of all the labels of the isolated vertices.
\end{proof}

In solving $g(n) = k$, therefore, it makes sense to consider first the ``prime`` $n$, that is, those which do not factorize nontrivially according to the above proposition: These will be called the \emph{connected} numbers, that is, the ones with connected generalized \hold graph. For a general $n$, we call $m$ the \emph{cyclic part} of $n$, and $n$ is \emph{cyclic-free} if $m = 1$. One application of connectivity is the following lower bound, obtained by considering non-nilpotent groups.
...
%\begin{proof}
%	We proceed by induction on $s,$ the case $s = 1$ being trivial. First, observe that this is equivalent to saying $\tilde{g}(n) \ge s - 1,$ where $\tilde{g}$ is the number of non-nilpotent groups. Now, having proved the result for $s,$ let $q$ be a new prime and $N = n q^\beta.$ We can form at least one nontrivial semidirect product because of connectivity, and this wil be our non-nilpotent group that completes the induction.
%\end{proof}
...
\begin{cor}\thlabel{eustick} Suppose $\Gamma$ is a \hold graph and $v$ is a disjoint vertex.\listspace
	\begin{enumerate} \listspace
...
\begin{lem}\thlabel{euinout}
	Let $r, s$ denote the in- and out-degree (respectively) of a vertex $v$ in a \hold graph $\Gamma$ with label $p$. Then $g(\Gamma) \ge 2^r + \frac{p^s - 1}{p - 1}$.
\end{lem}
\begin{proof}
...
Let $i(\Gamma), o(\Gamma)$ denote the maximum in- and out-degree of a vertex in a \hold graph $\Gamma$. If $o(\Gamma) = 3$, we get at least $1 + \frac{p^3 - 1}{p - 1} = p^2 + p + 2 \ge 8$. Thus we see that we must have $o(\Gamma) \le 2$, and for a similar reason $i(\Gamma) \le 2$. We therefore have four cases to consider.
...
We approach the problem as follows. Consider the coefficients of the $w$-terms in the formula in \thref{euppqr} of $g(n)$ for $n = p^2 q r$. Some of them depend on $n$ itself. If $n$ is such that the $w$-terms with nonconstant coefficients vanish, then once again $g(n)$ depends only on the \emph{generalized} \hold graph, so we call such $n$ regular as well. Consequently, it will suffice to enumerate all possible generalized \hold graphs of this type, and find a representative of each of them, and then use GAP to evaluate $g$. 
...
\subsubsection*{Three edges in the \hold graph}
