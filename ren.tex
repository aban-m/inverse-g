...
Using the Krull-Schmidt theorem {\cite{hungerford}}, one can see that $g(ab) \ge g(a)g(b)$. Since \mbox{$g(p^2) = 2$,} $g(p^3) = 5$ and $g(p^4) \ge 14$ for all primes $p$ (see, e.g., \cite[Th.~3.1]{gnumoas}), it is enough to consider fourth-power-free $n$, and in fact, we will see that $g(n)$ is rarely equal to 6 or 7 when $n$ is not cubefree.  The key computational tool will be directed graph: We associate a directed graph to each number, where the vertices are the prime powers dividing $n$ and the edges roughly indicate the existence of nontrivial semidirect products.
...
\case{C.1} \emph{$qrs$ is cyclic.} We assume without loss of generality that $q < r$. Moreover, it is clear that the out-degree of $p^2$ is at most 1 by \hthref{eubold} and \hold{1}'s formula. By connectivity, we have $q, r, s$ connected with $p^2$ (regardless of the directions of the arrows). We first assume that all the weaks involved are weak. This completely determines the graph: We must have $q, r \dashrightarrow p^2$ and $p^2 \dashrightarrow s$. Consequently, we have $g(p^2 s) = 4$ by \hthref{euppq} and $g(p^2 r s) = 5$ by our earlier analysis (the graph of $p^2 r s$ is ``isomorphic" to $\qlamd$). We claim that $g(n) = 7$ in this case, and it is then easy to see that replacing $p^2 s \dashrightarrow s$ by a strong arrow will increase $g$.

We observe that (where$\rtimes$ means a nontrivial semidirect product)
$$(\cyc{s} \rtimes \cyc{p}) \times \cyc{p},\enspace
\cyc{s} \times \cyc{p} \times \cyc{p},\enspace
\cyc{p^2} \times \cyc{s},\enspace
\cyc{s} \rtimes \cyc{p^2}$$
are pairwise nonisomorphic, and thus these are all the groups of order $p^2 s$. Similarly, the groups
$$(\cyc{s} \rtimes \cyc{p}) \times \cyc{p} \times \cyc{r},\enspace
\cyc{s} \times \cyc{p} \times \cyc{p} \times \cyc{r},\enspace
\cyc{p^2} \times \cyc{s} \times \cyc{r},\linebreak
\cyc{s} \rtimes \cyc{p^2} \times \cyc{r}\enspace
(\cyc{p^2} \rtimes \cyc{r}) \times \cyc{s}
$$
are all the groups of order $p^2 r s$. Multiplying each of these by $\cyc{q}$, we get $5$ groups of order $p^2 q r s$. In addition, the groups
$$(\cyc{p^2} \rtimes \cyc{q}) \times \cyc{s} \times \cyc{r},\enspace
(\cyc{p^2} \rtimes (\cyc{q} \times \cyc{r})) \times \cyc{s}$$
bring the total to $7$ groups of order $p^2 q r s$. We claim that $g(p^2 q r s) = 7$, i.e., that every group of order $G$ is isomorphic to one of these.

Let $G$ be of order $n = p^2 q r s$. Clearly, $n$ is odd and thus $G$ is solvable by Feit-Thompson {\cite{oddsolve}}. By Hall's theorems {\cite[Th.~3.13]{fgt}}, we have a subgroup $H_q$ of index $q$. By {\cite[Th.~5.14]{fgt}}, this subgroup is normal since its index is the smallest prime that divides $q$. Consequently, $G \cong H_q \rtimes \cyc{q}$.
It follows that there are $g(p^2 r s) = 5$ groups of order $n$ with a normal $q$-Sylow subgroup. We then assume that the $q$-Sylow subgroup is not normal.

First, by taking a $\{p, q\}$-Hall subgroup, it is easy to see that the $p$-Sylow subgroup is elementary abelian. Next, by using the solvability of $H_q$, we again construct a normal subgroup $H_{qr} \nsub H_q$ with $\abs{H_{qr}} = p^2 s$. Let $S$ be an $s$-Sylow subgroup of $H_{qr}$. We have $S \text{ char} H_{qr} \nsub H_q$ and thus $S \nsub H_q$, so $n_s(G) \mid q$. Since $q$ and $s$ are not related, we have $n_s(G) = 1$, i.e., the $s$-Sylow subgroup is normal. We claim that it is complement is normal as well. Suppose it is not, and write $G \cong S \rtimes K$ where $\abs{K} = p^2 q r$ is a Hall subgroup of $G$. Now consider the defining homomorphism $K \rightarrow \aut{K} \cong \cyc{s - 1}$. The kernel must have order $pqr$ or $qr$, and it is easy to see that the $q$-Sylow subgroup is characteristic in the kernel, and therefore normal in $K$. This forces $n_q(G) \mid s$ and consequently $n_q(G) = 1$, contradicting our assumption that the $q$-Sylow subgroup is not normal.

It follows that we have a correspondence between groups of order $n$ with their $q$-Sylow subgroup not normal, and groups of order $p^2 q r$ with their $q$-Sylow subgroup not normal. It is not hard to see that there are $g(p^2 q r) - g(p^2 r) = 5 - 3 = 2$ groups of this type, so we have $g(p^2 q r s) = 2 + g(p^2 r s) = 7,$ as claimed.
%
%
%
%

\case{C.2} \emph{$qrs$ has precisely one edge $q \rightarrow r$.} We claim that we have no admissible $n$ in this case.

For a set of primes $\alpha$, let $g^{(k)}_\alpha(n)$ denote the number of groups such that the Sylow subgroups corresponding to at least $k$ primes in $\alpha$ are direct factors (we can omit the superscript if $k = 1$). It is clear that \smash{$g^{(1)}_{\alpha \cup \beta}(n) = g^{(1)}_\alpha(n) + g^{(1)}_\beta(n) - g^{(2)}_{\alpha \cup \beta}(n),$} and $g_p(n) \coloneq g_{\{p\}}(n) = g(n/p^h)g(p^h)$, where $n = p^h n'$ and $n'$ is not divisible by $p$. The strategy will be to choose a suitable $\alpha$ and use these two facts to show that $g(n) > g_\alpha(n) \ge 7$ for all possible graphs of $n$. This will then prove our claim.

We need $g(p^2 s)g(qr) + 1 \le 7$, which forces $g(p^2 s) \le 3$. By connectivity, we have in fact $g(p^2 s) = 3$ and thus a weak arrow $s \dashrightarrow p^2$. Our graph of $n$ becomes connected by adding an edge. By considering $p^2 q r$, we immediately see that there are three options, corresponding to $M_1(q), \slame$ and $\qlame$:
\begin{center}
...
\case{C.3} \emph{qrs has precisely two edges}. Note that it cannot have more, because $g(p^2)g(qrs) + 1 \le 7$ and this forces $g(qrs) \le 3$.

At least one prime, say $s$, is connected to $p^2$. We have $g(p^2 s)g(qr) + 1 \le 7$ by connectivity, and this gives $g(p^2 s) = 3$ and thus we have an arrow $s \dashrightarrow p^2$. Changing the notation, we assume now that $q < r < s$. We cannot have an arrow $r \dashrightarrow s$ because then the graph of $p^2 r s$ will be a $M_1(r)$ and $r > 3$. If we have an arrow $q \dashrightarrow p^2$, we calculate $g_{\{p, s\}}(n) = 2g(qrs) + g(p^2 q r) - g(qr) = 6 + 6 - 2 = 10.$ If instead we have $s \dashrightarrow p^2$, we have $g_{\{q, p\}}(n) = g(p^2 r s) + 2g(qrs) - g(rs) = 9$.
%
%
%
%
%
% If $qrs$ is cyclic, we get $g(p^2 q r) \le 6$ precisely if the graph of $p^2 q r$ is $\qlamz$. An arrow $s \dashrightarrow p^2$ yields $2^3 = 8$ by a counting argument similar to the ones left, so we need $p^2 \rightarrow s$. We claim that we do have $g(n) = 7$ in this case, making use of the observation $g(p^2 q r) = g(p^2 q s) = 5$.
...
\begin{thm}
	...
	\end{enumerate} \textspace
	where in cases I--VII no further congruences of the form $\alpha \equiv \pm 1 \Mod{\beta}$ \nolinebreak[4] \mbox{occur}.
\end{thm}
