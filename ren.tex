...
\end{lem}
\begin{proof}
	The only thing that is not obvious is the first form. \textit{A priori}, we only know that $n$ is a divisor of $p^2 q_1 \cdots q_5$, since $g(p^2) + 5 = 7$. We need $g(q_1 \cdots q_s) \le 7/2 = 3.5$ thus the \hg{1} of $q_1 \cdots q_5$ has at most 3 edges. This means that at least one vertex $q_5$ is disconnected, so we can discard. If the graph of $q_1 \cdots q_4$ is connected, we need at least one vertex of either out- or in-degree $2$, yielding at least 4 groups, which contradicts our assumption. Thus there must be at most three vertices.
\end{proof}
...
\begin{thm}
	...
	\begin{enumerate}
		\item If $g(n) = 2$, then either $n = p^2$ or $n = pq$ where $q \equiv 1 \Mod{p}$.
		\item If $g(n) = 3$, then either $n = p^2 q$ and $q \mid p + 1$, or $n = pqr$ where $q \equiv 1 \Mod{p}$ and $r \equiv 1 \Mod{q}$ and $r \not\equiv 1 \Mod{p}$.
	\end{enumerate}
\end{thm}
