...
Using the Krull-Schmidt theorem {\cite{hungerford}}, one can see that $g(ab) \ge g(a)g(b)$. Since \mbox{$g(p^2) = 2$,} $g(p^3) = 5$ and $g(p^4) \ge 14$ for all primes $p$ (see, e.g., \cite[Th.~3.1]{gnumoas}), it is enough to consider fourth-power-free $n$, and in fact, we will see that $g(n)$ is rarely equal to 6 or 7 when $n$ is not cubefree. so we will mostly be working with cubefree $n$. The key computational tool will be directed graphs: We represent numbers by graphs to facilitate the analysis of \hold{1}-type formulas. The vertices will be the prime powers dividing $n$, and the edges will essentially indicate that nontrivial semidirect products exist.
