...
We introduce the functionn $g^{(k)}_\alpha$ for a set of primes $\alpha$, counting the number of groups such that the Sylow subgroups corresponding to at least $k$ primes in $\alpha$ are direct factors (we omit the superscript if $k = 1$). It is clear that $g^{(1)}_{\alpha \cup \beta}(n) = g^{(1)}_\alpha(n) + g^{(1)}_\beta(n) - g^{(2)}_{\alpha \cup \beta}(n)$, and $g_p(n) \coloneq g_{\{p\}}(n) = g(n/p^h)$, where $n = p^h n'$ and $n'$ is not divisible by $p$. The strategy will be to choose suitable $\alpha$ and use these two facts to show that $g(n) > g_\alpha(n) \ge 7$, proving our claim.
