...
%
%
\case{C.2} \emph{$qrs$ has precisely one edge $q \rightarrow r$.}
We claim that we have no admissible $n$ in this case. We introduce the notation $g_{\alpha}(n)$ for a set $\alpha$ of primes to mean the number of groups of order $n$ satisfying the condition that the $p$-Sylow subgroup is a direct factor whenever $p$ is in $\alpha$. Observe that $g(n) > g_\alpha(n)$ whenever $\alpha$ is nonempty set of prime factors of $n$ and $n$ is connected. The strategy is to compute $g_\alpha(n)$ for a suitably chosen subset $\alpha$ and show that the result is at least $7$, meaning that $g(n) \ge 8$, showing that $n$ is inadmissible.
...
\resizebox{\textwidth}{!}{%
	\begin{tikzpicture}[scale=0.9]
		\begin{scope}[shift=(0, 0)]
			\node[point, label=up:$s$]							(s1) {};
			\node[point, label=up:$p^2$, right of=s1]			(p1) {};
			\node[point, label=bottom:$q$, below of=s1]			(q1) {};
			\node[point, label=bottom:$r$, below of=p1]			(r1) {};
		\end{scope}
		
		\begin{scope}[shift=(4, 0)]
			\node[point, label=up:$s$]							(s2) {};
			\node[point, label=up:$p^2$, right of=s1]			(p2) {};
			\node[point, label=bottom:$q$, below of=s1]			(q2) {};
			\node[point, label=bottom:$r$, below of=p1]			(r2) {};		
		\end{scope}
		
		\begin{scope}[shift=(8, 0)]
			\node[point, label=up:$s$]							(s3) {};
			\node[point, label=up:$p^2$, right of=s1]			(p3) {};
			\node[point, label=bottom:$q$, below of=s1]			(q3) {};
			\node[point, label=bottom:$r$, below of=p1]			(r3) {};
		\end{scope}
		...
		% \path[->, thick] (p1) edge[dashed] (q1);
		% \path[->, thick] (p4) edge[dashed] (q4);
		% \path[->, thick] (p7) edge (q7);		% THE IMAGE GOES HERE.
	\end{tikzpicture}
}
