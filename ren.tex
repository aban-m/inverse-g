...
Having dealt with the `irregular` cases, we consider regular $n$ with two edges in its graph, and calculate $g$ for one representative using GAP. 
...
\begin{center}
	\begin{tikzpicture}
		...
		\begin{scope}[shift={(0, 2.8)}]
			...
			\node[point, label=left:$r$, below=of p2] (q2) {};
			\node[point, label=right:$r$] at ($(q2) + (22.5:2)$)(r2) {};
			\node at ($(r2) + (230:1.6)$) {LIMBO};
		\end{scope}
		...
		(q4) edge[dashed] (p4) (q4) edge (r4)
		(q6) edge[dashed]  (p6) (r6) edge[dashed] (p6);
