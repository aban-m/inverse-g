...
% finishing Holder case work in lambda(n) = 4 [requires TikZ]
...
\begin{document}
\title{Some Values and Fibers of the Group-counting Function}
\author{Aban}
\maketitle
...
\begin{fact}[\textbf{Holder's formula}]\thlabel{euholder} In this notation, we have
	\begin{equation*}
...
%\emph{Remark.} From Dirichlet's theorem on arithmetic progression see that every directed acyclic graph is the Holder graph of infinitely many squarefree integers: Ultimately, choosing the next prime in the factorization amounts to finding a prime satisfying some linear congruences modulo primes, and this is always possible.
...
\begin{proof}
	...
	Finally, if $\qo \notin \piga$ and $\pin \notin \pila,$ the previous analysis shows that\linebreak $h_M(\qo, \tilpi) = 0$, which in turn implies that $S(\tilpi, M) = 0$ and so $\tilpi$ contributes nothing to the sum.
...
\begin{cor}\thlabel{eustick} Suppose $\Gamma$ is a Holder graph and $v$ is a disjoint vertex.\listspace
	\begin{enumerate} \listspace
...
\case{4} $i(\Gamma) = 2, o(\Gamma) = 2$. A vertex labeled $p$ with in-degree and out-degree both equal to $2$ would yield $4 + p + 1 = p + 5$ groups, but we cannot have $p = 2$ because it is not terminal. So let $u$ be the unique vertex with out-degree $2$ and label it $q$, and assume we have $u \rightarrow v_1, v_2$, and call this graph $Q$. We have already seen that $g(Q) = p + 2$. We can add a new edge $v_1 \rightarrow v_2$ which will give $p + 3$ groups. Adding a new vertex $w$ instead gives us the option to join $w$ to either $v_1$ or $v_2$, but this is not admissible: The subsets $\{v_1, v_2\}$ and $\{w, v_1, v_w\}$ will both be central, and will contribute $2(p + 1) = 2p + 2 \ge 8$ groups in total. Now an arrow $v_1 \rightarrow w$ would reduce this to case 3, and it yields $p + 4 \ge 7$ groups, and we certainly cannot add any more edges, and this contradicts the $i(\Gamma) = 2$ condition. Thus the only case is the one obtained by adding an edge $v_1 \rightarrow v_2$ to $Q$ for $p = 3$. 
...
\begin{thm}
	...
	\begin{enumerate} \listspace
		...
		\end{enumerate}
	\end{enumerate} \textspace
	and in cases I, II, VI, VII no further congruences of the form $\alpha \equiv \pm 1 \Mod{\beta}$ \nolinebreak[4] \mbox{occur}.
\end{thm}
\begin{thm}
	...
	\end{enumerate} \textspace
	and in cases I, II, V, VI no further congruences of the form $\alpha \equiv \pm 1 \Mod{\beta}$ \nolinebreak[4] \mbox{occur}.
\end{thm}
